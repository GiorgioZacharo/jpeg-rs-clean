...
void IZigzagMatrix_f2r_forBody_s2e_forEnd(int imatrix[DCTSIZE2], int omatrix[DCTSIZE2]) {
  ...
  /* initialize HP, ACP, and HPC accelerators */
#ifdef __HPC_ACCELERATOR__
  init_accelerator_coherency(HPC_CONTROLLER_MEM_ADDR);
#endif
  ...
  XIzigzagmatrix_f2r_forbody_s2e_forend_Set_imatrix(&Izigzagmatrix_f2r_forbody_s2e_forend_hpc, (UINTPTR)imatrix);
  XIzigzagmatrix_f2r_forbody_s2e_forend_Set_omatrix(&Izigzagmatrix_f2r_forbody_s2e_forend_hpc, (UINTPTR)omatrix);
  
  XIzigzagmatrix_f2r_forbody_s2e_forend_Start(&Izigzagmatrix_f2r_forbody_s2e_forend_hpc);

  while (!XIzigzagmatrix_f2r_forbody_s2e_forend_IsDone(&Izigzagmatrix_f2r_forbody_s2e_forend_hpc));
...
IQuantize (int *matrix, unsigned int *qmatrix){
  ...
  XIquantize_Set_qmatrix(&Iquantize_hpc, (UINTPTR)qmatrix);
  XIquantize_Set_matrix(&Iquantize_hpc, (UINTPTR)matrix);


  XIquantize_Start(&Iquantize_hpc);

  while (!XIquantize_IsDone(&Iquantize_hpc));
...
//  Xil_DCacheFlushRange((UINTPTR)rgb_buf, 768 * sizeof(int));
  ...
  XDecode_start_f2r_vectorph_s2e_forbody96preheader_Set_y_buf(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc, (UINTPTR)y_buf);
  XDecode_start_f2r_vectorph_s2e_forbody96preheader_Set_u_buf(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc, (UINTPTR)u_buf);
  XDecode_start_f2r_vectorph_s2e_forbody96preheader_Set_v_buf(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc, (UINTPTR)v_buf);
  XDecode_start_f2r_vectorph_s2e_forbody96preheader_Set_rgb_buf(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc, (UINTPTR)rgb_buf);


  XDecode_start_f2r_vectorph_s2e_forbody96preheader_Start(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc);

  while (!XDecode_start_f2r_vectorph_s2e_forbody96preheader_IsDone(&Decode_start_f2r_vectorph_s2e_forbody96preheader_hpc));
