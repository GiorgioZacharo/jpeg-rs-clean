...
//------------------------Parameter----------------------
`define TV_IN_store "../tv/cdatafile/c.WriteOneBlock_f2r_entry_s2e_forEnd13.autotvin_store.dat"
`define TV_IN_out_buf "../tv/cdatafile/c.WriteOneBlock_f2r_entry_s2e_forEnd13.autotvin_out_buf.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter store_DEPTH = 1;
reg [31 : 0] store_OPERATE_DEPTH = 1;
parameter store_c_bitwidth = 64;
parameter out_buf_DEPTH = 1;
reg [31 : 0] out_buf_OPERATE_DEPTH = 1;
parameter out_buf_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter WriteOneBlock_f2r_entry_s2e_forEnd13_continue_addr = 0;
parameter WriteOneBlock_f2r_entry_s2e_forEnd13_auto_start_addr = 0;
parameter store_data_in_addr = 16;
parameter out_buf_data_in_addr = 28;
...
input    [64 - 1 : 0] TRAN_store;
input    [64 - 1 : 0] TRAN_out_buf;
...
reg  BREADY_reg = 0;
reg [store_c_bitwidth - 1 : 0] reg_store;
reg store_write_data_finish;
reg [out_buf_c_bitwidth - 1 : 0] reg_out_buf;
reg out_buf_write_data_finish;
...
//write store reg
reg [31 : 0] write_store_count = 0;
reg write_store_run_flag = 0;
reg write_one_store_data_done = 0;
//write out_buf reg
reg [31 : 0] write_out_buf_count = 0;
reg write_out_buf_run_flag = 0;
reg write_one_out_buf_data_done = 0;
...
assign TRAN_BUS_CTRL_write_data_finish = 1 & store_write_data_finish & out_buf_write_data_finish;
...
always @(TRAN_store) 
begin
    reg_store = TRAN_store;
end
always @(TRAN_out_buf) 
begin
    reg_out_buf = TRAN_out_buf;
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        store_write_data_finish <= 0;
        write_store_run_flag <= 0; 
        write_store_count = 0;
        count_operate_depth_by_bitwidth_and_depth (store_c_bitwidth, store_DEPTH, store_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_store_run_flag <= 1; 
            write_store_count = 0;
        end
        if (write_one_store_data_done === 1) begin
            write_store_count = write_store_count + 1;
            if (write_store_count == store_OPERATE_DEPTH) begin
                write_store_run_flag <= 0; 
                store_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_store
    integer write_store_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] store_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = store_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_store_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write store data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (store_c_bitwidth < 32) begin
                        store_data_tmp_reg = reg_store;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < store_c_bitwidth) begin
                                store_data_tmp_reg[j] = reg_store[i*32 + j];
                            end
                            else begin
                                store_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (store_data_in_addr + write_store_count * four_byte_num * 4 + i * 4, store_data_tmp_reg, write_store_resp);
                end
                process_busy = 0;
                write_one_store_data_done <= 1;
                @(posedge clk);
                write_one_store_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        out_buf_write_data_finish <= 0;
        write_out_buf_run_flag <= 0; 
        write_out_buf_count = 0;
        count_operate_depth_by_bitwidth_and_depth (out_buf_c_bitwidth, out_buf_DEPTH, out_buf_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_BUS_CTRL_start_in === 1) begin
            out_buf_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_out_buf_run_flag <= 1; 
            write_out_buf_count = 0;
        end
        if (write_one_out_buf_data_done === 1) begin
            write_out_buf_count = write_out_buf_count + 1;
            if (write_out_buf_count == out_buf_OPERATE_DEPTH) begin
                write_out_buf_run_flag <= 0; 
                out_buf_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_out_buf
    integer write_out_buf_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] out_buf_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = out_buf_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_out_buf_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write out_buf data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (out_buf_c_bitwidth < 32) begin
                        out_buf_data_tmp_reg = reg_out_buf;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < out_buf_c_bitwidth) begin
                                out_buf_data_tmp_reg[j] = reg_out_buf[i*32 + j];
                            end
                            else begin
                                out_buf_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (out_buf_data_in_addr + write_out_buf_count * four_byte_num * 4 + i * 4, out_buf_data_tmp_reg, write_out_buf_resp);
                end
                process_busy = 0;
                write_one_out_buf_data_done <= 1;
                @(posedge clk);
                write_one_out_buf_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_start_count >= 288) begin
            write_start_run_flag <= 0; 
        end
