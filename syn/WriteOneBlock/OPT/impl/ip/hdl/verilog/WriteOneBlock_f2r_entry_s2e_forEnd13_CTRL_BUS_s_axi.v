...
#(parameter
    ...
    output wire [31:0]                   voffs,
    output wire [31:0]                   hoffs
);
//------------------------Address Info-------------------
// 0x00 : reserved
// 0x04 : reserved
...
localparam
    ...
    ADDR_VOFFS_DATA_0  = 6'h20,
    ADDR_VOFFS_CTRL    = 6'h24,
    ADDR_HOFFS_DATA_0  = 6'h28,
    ADDR_HOFFS_CTRL    = 6'h2c,
    WRIDLE             = 2'd0,
    WRDATA             = 2'd1,
    WRRESP             = 2'd2,
    WRRESET            = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_voffs = 'b0;
    reg  [31:0]                   int_hoffs = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_VOFFS_DATA_0: begin
                    rdata <= int_voffs[31:0];
                end
                ADDR_HOFFS_DATA_0: begin
                    rdata <= int_hoffs[31:0];
                end
            endcase
        end
    end
...
assign voffs  = int_voffs;
assign hoffs  = int_hoffs;
// int_width[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_width[31:0] <= 0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VOFFS_DATA_0)
            int_voffs[31:0] <= (WDATA[31:0] & wmask) | (int_voffs[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_HOFFS_DATA_0)
            int_hoffs[31:0] <= (WDATA[31:0] & wmask) | (int_hoffs[31:0] & ~wmask);
    end
end
