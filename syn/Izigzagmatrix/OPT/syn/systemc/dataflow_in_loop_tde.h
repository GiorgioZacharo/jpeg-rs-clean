...
#ifndef _dataflow_in_loop_tde_H_
#define _dataflow_in_loop_tde_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "dataflow_in_loop_tde_memcore.h"

SC_MODULE(dataflow_in_loop_tde) {
...
protected:
    ...
    dataflow_in_loop_tde_memcore* dataflow_in_loop_tde_memcore_U;
...
public:
    ...
    SC_CTOR(dataflow_in_loop_tde) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        dataflow_in_loop_tde_memcore_U = new dataflow_in_loop_tde_memcore("dataflow_in_loop_tde_memcore_U");
        dataflow_in_loop_tde_memcore_U->clk      ( clk );
        dataflow_in_loop_tde_memcore_U->reset    ( reset );
        dataflow_in_loop_tde_memcore_U->ce0      ( i_ce0 );
        dataflow_in_loop_tde_memcore_U->we0      ( i_we0 );
        dataflow_in_loop_tde_memcore_U->address0 ( memcore_iaddr );
        dataflow_in_loop_tde_memcore_U->d0       ( i_d0 );
        dataflow_in_loop_tde_memcore_U->q0       ( i_q0 );

        dataflow_in_loop_tde_memcore_U->ce1      ( t_ce0 );
        dataflow_in_loop_tde_memcore_U->we1      ( t_we0 );
        dataflow_in_loop_tde_memcore_U->address1 ( memcore_taddr );
        dataflow_in_loop_tde_memcore_U->d1       ( t_d0 );
        dataflow_in_loop_tde_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "dataflow_in_loop_tde.trace");
            // ports
