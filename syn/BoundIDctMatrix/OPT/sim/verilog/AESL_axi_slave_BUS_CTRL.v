...
//------------------------Parameter----------------------
`define TV_IN_matrix "../tv/cdatafile/c.BoundIDctMatrix.autotvin_matrix.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter matrix_DEPTH = 1;
reg [31 : 0] matrix_OPERATE_DEPTH = 1;
parameter matrix_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter BoundIDctMatrix_continue_addr = 0;
parameter BoundIDctMatrix_auto_start_addr = 0;
parameter matrix_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [matrix_c_bitwidth - 1 : 0] reg_matrix;
reg matrix_write_data_finish;
...
//write matrix reg
reg [31 : 0] write_matrix_count = 0;
reg write_matrix_run_flag = 0;
reg write_one_matrix_data_done = 0;
...
assign TRAN_BUS_CTRL_write_data_finish = 1 & matrix_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        matrix_write_data_finish <= 0;
        write_matrix_run_flag <= 0; 
        write_matrix_count = 0;
        count_operate_depth_by_bitwidth_and_depth (matrix_c_bitwidth, matrix_DEPTH, matrix_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_matrix_run_flag <= 1; 
            write_matrix_count = 0;
        end
        if (write_one_matrix_data_done === 1) begin
            write_matrix_count = write_matrix_count + 1;
            if (write_matrix_count == matrix_OPERATE_DEPTH) begin
                write_matrix_run_flag <= 0; 
                matrix_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_matrix
    integer write_matrix_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] matrix_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = matrix_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_matrix_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write matrix data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (matrix_c_bitwidth < 32) begin
                        matrix_data_tmp_reg = reg_matrix;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < matrix_c_bitwidth) begin
                                matrix_data_tmp_reg[j] = reg_matrix[i*32 + j];
                            end
                            else begin
                                matrix_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (matrix_data_in_addr + write_matrix_count * four_byte_num * 4 + i * 4, matrix_data_tmp_reg, write_matrix_resp);
                end
                process_busy = 0;
                write_one_matrix_data_done <= 1;
                @(posedge clk);
                write_one_matrix_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
