...
BoundIDctMatrix (int *matrix, int Bound) {
  ...
  #ifdef VERBOSE
  printf("Call HW REG_8 - BoundIDctMatrix\n");
  #endif
  ...
  // Flush the cache of the buffers
  Xil_DCacheFlushRange((UINTPTR)matrix, DCTSIZE2 * sizeof(int));
//  Xil_DCacheFlushRange((UINTPTR)matrix,  sizeof(int));

  XBoundidctmatrix_Set_matrix(&Boundidctmatrix, (UINTPTR)matrix);
  XBoundidctmatrix_Set_Bound(&Boundidctmatrix, (UINTPTR)Bound);


  XBoundidctmatrix_Start(&Boundidctmatrix);

  while (!XBoundidctmatrix_IsDone(&Boundidctmatrix));
  ...
  for (i = 0; i < N_CHUNKS; i++){
    ...
    // Store Data
      ...
      for (int l=0; l < CHUNK_SIZE; l++)
      #pragma HLS UNROLL skip_exit_check factor=16
      matrix[i*CHUNK_SIZE+l] = inp1_buf[l];
    #else
      offset = i*CHUNK_SIZE;
...
void WriteOneBlock_f2r_entry_s2e_forEnd13(int store[DCTSIZE2], unsigned char out_buf[5310], int width, int height,
  ...
  #ifdef VERBOSE
  printf("Call HW REG_4 - WriteOneBlock\n");
  #endif
  ...
  // Flush the cache of the buffers
  Xil_DCacheFlushRange((UINTPTR)store, DCTSIZE2 * sizeof(int));
  Xil_DCacheFlushRange((UINTPTR)out_buf, 5310 * sizeof(unsigned char));

  XWriteoneblock_f2r_entry_s2e_forend13_Set_store(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)store);
  XWriteoneblock_f2r_entry_s2e_forend13_Set_out_buf(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)out_buf);
  XWriteoneblock_f2r_entry_s2e_forend13_Set_width(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)width);
  XWriteoneblock_f2r_entry_s2e_forend13_Set_height(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)height);
  XWriteoneblock_f2r_entry_s2e_forend13_Set_voffs(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)voffs);
  XWriteoneblock_f2r_entry_s2e_forend13_Set_hoffs(&Writeoneblock_f2r_entry_s2e_forend13, (UINTPTR)hoffs);


  XWriteoneblock_f2r_entry_s2e_forend13_Start(&Writeoneblock_f2r_entry_s2e_forend13);

  while (!XWriteoneblock_f2r_entry_s2e_forend13_IsDone(&Writeoneblock_f2r_entry_s2e_forend13));

  // Invalidate the cache to avoid reading garbage
  Xil_DCacheInvalidateRange((UINTPTR)out_buf, 5310 * sizeof(unsigned char));
